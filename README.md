...
### 🔧 重要修复：人脸图片过滤问题 (2025-11-12)

**问题描述**：之前的版本中，虽然定义了人脸检测相关的过滤逻辑，但在实际过滤条件中没有使用，导致包含人脸的图片仍然被下载。

**修复内容**：
- ✅ **主程序修复**：在 `xiaohongshu-scraper.js` 中，将 `isNotPerson` 检查添加到过滤条件中
- ✅ **扩展修复**：在 `content.js` 中，新增完整的图片过滤函数 `shouldFilterImage()`
- 🎯 **增强过滤**：
  - 添加人脸相关关键词检测：`person`, `people`, `selfie`, `portrait`, `face`, `headshot`
  - 添加图标容器检测：排除在 `.avatar`, `.icon`, `.user-head` 等容器中的图片
  - 添加DOM结构检测：确保图片在内容区域内（`.note-item`, `.feed-item`, `.article`等）
  - 添加尺寸和比例检测：排除正方形小图标（宽高比接近1:1且尺寸小于200x200）

**过滤策略**（两层过滤机制）：
1. **第一层：URL关键词过滤**（快速、粗略）
   - 检查URL中是否包含头像、系统、人脸等关键词
   - 检查图片尺寸是否符合要求（>200x200）
   - 检查图片长宽比，排除明显的头像图标
   - 检查DOM结构，排除非内容区域的图片

2. **第二层：人脸检测API**（精确但可选）
   - Chrome扩展中可启用"过滤人脸图片"选项
   - 使用浏览器原生的 Face Detection API
   - 对通过第一层过滤的图片进行人脸检测
   - 只保留不包含人脸的图片

**使用建议**：
- 主程序和Chrome扩展都已自动启用URL关键词过滤
- Chrome扩展用户建议启用"过滤人脸图片"选项，效果更佳
- 如果发现仍有人脸图片，请反馈具体的URL特征，我们会继续改进过滤规则

---

### 🆕 功能增强：智能排序功能 (2025-11-12)

**新增功能**：自动按"最热"（最多点赞）排序搜索结果

**功能说明**：
- 🎯 **双重保障策略**：点击UI按钮（主要） + URL参数排序（备用）
- 📊 **提升质量**：优先展示最受欢迎的内容，确保下载高质量图片
- 🔄 **自动执行**：无需手动操作，系统自动完成排序设置
- ✅ **兼容性**：同时支持主程序和Chrome扩展
- 🎨 **更符合用户习惯**：优先使用UI交互方式

**技术实现**（执行流程）：
1. **步骤1：点击"筛选"按钮**
   - 根据小红书页面结构，"最多点赞"选项在筛选菜单中
   - 先点击右上角的"筛选"按钮打开筛选菜单
   - 多种查找方式：文本匹配、DOM遍历
   
2. **步骤2：点击"最多点赞"选项**
   - 在筛选菜单中查找"最多点赞"选项
   - 优先在筛选容器（`modal`, `popup`, `menu`）中查找
   - 多种点击方式保障成功率（直接点击、事件触发）
   
3. **备用方案：URL参数排序**
   - 当UI点击失败时自动启用
   - 在URL中添加 `sort_type=hot` 参数
   - 直接通过URL控制排序
   - 确保排序功能一定生效
...
- 确保搜索类型为图文：`type=51`
- 图片过滤尺寸要求：200x200
- 多层过滤逻辑：URL关键词过滤 + 人脸检测API（可选）

**使用说明**：
- 功能已自动启用，无需额外配置
- 系统会先尝试点击"最多点赞"按钮
- 如果按钮点击失败，自动切换到URL参数方式
- 可以在Chrome扩展中启用"过滤人脸图片"选项，进一步提升图片质量
- 推荐配合人脸检测功能使用，效果更佳

---

### ✅ 修复：登录后任务不继续执行的问题 (2025-11-09)

**问题描述**：在Web界面点击"开始下载"后，Chromium浏览器打开并完成扫码登录，但下载任务没有继续执行。

**修复内容**：
- ⚡ **核心修复**：增加Cookie同步机制，登录成功后自动将Cookie同步到主页面
- ⏱️ 增加初始等待时间到60秒，给用户更充足的时间完成扫码
- 📢 优化用户提示，每个阶段都有清晰的反馈信息
- 🔄 每15秒提示登录进度，避免用户误以为程序卡死
...
**详细说明**：
- 📖 [用户友好的修复说明](./修复说明.md) - **推荐阅读**
- 🔧 [技术文档](./FIX_LOGIN_ISSUE.md) - 开发者参考
...
### 🔍 核心功能
- 根据餐馆名称和地点智能搜索
- **🏆 智能排序**：自动按"最热"排序搜索结果（URL参数方式），优先获取最受欢迎的图片
- 自动下载餐馆相关图片
- 支持批量下载
- 自动创建以餐馆名称命名的文件夹管理图片
- 内置反爬虫检测处理
...
3. **CSV导入简化**：导入CSV文件时只需要"餐馆名称,地点(可选)"两列，图片数统一使用全局配置
4. **实时显示**：餐馆列表中显示的图片数会实时反映全局配置的数值

### 配置方式
- **全局设置**：在"配置管理"区域设置"每个餐馆下载的图片数"
- **CSV格式**：导入文件格式为"餐馆名称,地点(可选)"（图片数由全局配置控制）
...
### 功能优化
- 🏷️ **地点字段可选化**：地点字段不再是必须项，用户可以选择不填写地点信息
- 📝 **界面优化**：移除地点字段的必填标记（*）和required属性
- 📊 **CSV导入优化**：CSV导入时地点字段变为可选，支持只填写餐馆名称
- 🔍 **显示优化**：当地点为空时显示"未设置地点"，提供更好的用户体验
- 📚 **文档更新**：更新README.md和相关说明，明确地点字段的可选性质

### 技术实现
- **HTML修改**：移除地点字段的required属性和星号标记
- **JavaScript验证**：修改保存餐馆配置的验证逻辑，只验证餐馆名称
- **CSV解析优化**：修改CSV解析逻辑，支持地点为空的情况
- **显示逻辑优化**：在餐馆列表和进度显示中处理地点为空的情况
- **错误信息更新**：更新相关错误提示信息，反映地点字段的可选性质

### 使用说明
- **手动添加餐馆**：现在只需要填写餐馆名称，地点可以留空
- **CSV导入**：支持格式为"餐馆名称,地点(可选)"的CSV文件
- **显示效果**：未设置地点的餐馆会显示"未设置地点"标识
- **向后兼容**：现有的包含地点的配置仍然正常工作
...
**注意事项**：
- CSV格式：`餐馆名称,地点(可选),下载的图片数(可选)`
- 每行必须包含餐馆名称，地点为可选，下载的图片数为可选（默认为6）
...
**解决方案**：
1. **去除格式标识词**：修改评语生成提示词，明确要求不输出"标题:"、"正文:"、"结尾标签:"等格式标识词
2. **直接输出纯净内容**：要求AI直接输出评语内容，格式为标题、正文、结尾标签的自然分段
3. **保持评语质量**：在去除格式标识词的同时，保持评语的真实性和自然性

**技术实现**：
- 更新`config.template.json`中的`reviewPrompt`模板，添加输出格式要求
- 更新`public/js/app.js`中的前端评语提示词配置
- 更新`tests/test-review-generation.js`中的测试评语提示词
- 在提示词中明确要求："直接输出评语内容，不要包含'标题:'、'正文:'、'结尾标签:'等格式标识词"
...
**问题描述**：用户反馈评语生成存在以下问题：
1. 缺少大众点评活动标签（#创作者赏金计划 #0元玩转这座城 #城市向导官# 优质创作者赏金计划）
2. 评语中重复出现固定语句"店里人挺多的，饭点可能要等位，价格不算便宜，但用料确实不错"
3. 评语词汇太平乏，缺乏多样性
4. 没有结合网上查到的店家特色菜信息

**解决方案**：
1. **添加活动标签**：在评语结尾标签中强制包含大众点评活动标签
2. **避免重复语句**：更新提示词，明确禁止重复使用固定语句，要求使用不同的表达方式
3. **增加词汇多样性**：提供更多变化的中性描述选项
4. **集成特色菜研究**：新增餐馆特色菜研究模块，自动获取店家招牌菜信息并融入评语生成

**技术实现**：
- 更新评语生成提示词模板，添加活动标签要求和避免重复语句的指导
- 创建`src/restaurant-research.js`模块，提供餐馆特色菜研究功能
- 更新`src/ai-service.js`，集成特色菜研究功能到AI分析流程
- 修改评语生成逻辑，将特色菜信息融入提示词中
- 更新配置文件`config.template.json`和测试文件`test-review-generation.js`
