...
async function checkBatchBrowser() {
    console.log('🔧 正在检查批量处理流程中的浏览器窗口状态...');
    console.log('💡 这个工具会检查现有的浏览器实例，并确保窗口可见');
    
    try {
        // 1. 检查现有的浏览器实例
        console.log('🔍 正在检查现有的浏览器实例...');
        ...
        try {
            // 这里可以尝试连接到现有的浏览器实例
            // 由于安全限制，我们暂时创建新的实例
            console.log('⚠️ 无法连接到现有浏览器实例，将创建新的实例');
        } catch (error) {
            console.log('⚠️ 无法连接到现有浏览器实例，将创建新的实例');
        }
        ...
        console.error('❌ 检查批量处理流程中的浏览器窗口状态失败:', error.message);
